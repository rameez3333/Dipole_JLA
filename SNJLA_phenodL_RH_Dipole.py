...
ofname = 'FOuts/OutQKinPhenodL_RH_D' + str(options.DET)
...
def RESVF3RH1Dip( Q0, J0 , A , B , M0, X0, C0, X1, C1, X2, C2, X3, C3, QD, DS=np.inf, stype = STYPE ): #Total residual, \hat Z - Y_0*A
    YA={}
    YA[1.] = np.array([ M0-A*X0+B*C0, X0, C0])
    YA[2.] = np.array([ M0-A*X1+B*C1, X1, C1])
    YA[3.] = np.array([ M0-A*X2+B*C2, X2, C2])
...
def m2loglike(pars , RV = 0):
    if RV != 0 and RV != 1 and RV != 2:
        ...
        if options.DET<2:
            res = RESVF3RH1( *[ pars[i] for i in [0,1,2,5,8,3,6, 10, 11, 12, 13, 14, 15] ] )
        elif options.DET==2:
            res = RESVF3RH1Dip( *[ pars[i] for i in [0,1,2,5,8,3,6, 10, 11, 12, 13, 14, 15, 16] ] )
        elif options.DET>=3:
            res = RESVF3RH1Dip( *[ pars[i] for i in [0,1,2,5,8,3,6, 10, 11, 12, 13, 14, 15, 16, 17] ] )
...
pre_found_best = np.array([ -3.13971076e-01,   3.72791401e-02,   1.34264121e-01,
        ...
        -1.60647458e-02,   5.04579623e-03,  -1.90471016e+01,
         1.17220199e-02, 3.80474258e-02, -1.60647458e-02,3.80474258e-02, -1.60647458e-02, 3.80474258e-02, -1.60647458e-02])
